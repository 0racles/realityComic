define([''], function () { 
	...
	showScreen = function (event) {
		...
		if (event.target.classList.contains("comicScreen1")) {
			...
			body.setAttribute("background", "..\/realityComic\/assets\/img\/londonCountrySide1.JPG");
			imageOpacityHigh(spiderman[0]);
			$(lab[0]).show()
			$(spiderman[0]).show("drop", 5000);
			makeWells();
			colorLabel(event.target);
			//$('[data-toggle="popover"]').popover(); // remember to remove after a wnile
	...
	addEventHandlers = function () {
		...
		//google.maps.event.addDomListener(window, "load", initializeGM); // memo: remember to remove the comment
